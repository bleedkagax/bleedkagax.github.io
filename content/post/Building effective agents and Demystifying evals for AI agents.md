...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| • **String match checks**：字符串匹配检查（完全匹配、正则匹配、模糊匹配等） <br><br>• **Binary tests**：二元测试（失败→通过、通过→仍通过）  <br><br>• **Static analysis**：静态分析（代码规范、类型检查、安全检查）  <br><br>• **Outcome verification**：结果验证  <br><br>• **Tool calls verification**：工具调用验证（使用的工具、参数）  <br><br>• **Transcript analysis**：对话记录分析（交互轮数、Token 使用量） | • 快  <br><br>• 成本低 <br><br>• 客观  <br><br>• 可复现  <br><br>• 易于调试  <br><br>• 可验证特定条件<br><br><br><br><br> | • 对于不完全符合预期模式但实际合理的变体很脆弱  <br><br>• 缺乏精细度  <br><br>• 对一些更主观的任务评估能力有限 |

Model-based graders

| **Methods**                                                                                                                                                                                                                    | **Strengths**                                                                 | **Weaknesses**                                              |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ----------------------------------------------------------- |
| • **Rubric-based scoring**：基于评分标准的打分  <br><br>• **Natural language assertions**：自然语言断言  <br><br>• **Pairwise comparison**：成对比较  <br><br>• **Reference-based evaluation**：基于参考答案的评估  <br><br>• **Multi-judge consensus**：多评审一致性 | • 灵活  <br><br>• 可扩展  <br><br>• 能捕捉细微差别<br><br>• 能处理开放式任务  <br><br>• 能处理自由格式输出 | • 非确定性  <br><br>• 成本高于代码评分  <br><br>• 需要与人工评分进行校准以保证准确性<br> |
|                                                                                                                                                                                                                                |                                                                               |                                                             |
Human graders

| **Methods**                                                                                                                                                                                                                                                 | **Strengths**                                                           | **Weaknesses**                           |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------- |
| • **SME review**：领域专家评审（Subject Matter Expert）  <br><br>• **Crowdsourced judgment**：众包评判  （把评估任务分发给大量普通标注者）<br><br>• **Spot-check sampling**：抽样检查  <br><br>• **A/B testing**：A/B 测试  <br><br>• **Inter-annotator agreement**：标注者一致性（看多个标注者对同一结果的判断是否一致）<br> | • 高标准质量 <br><br>• 与专家用户判断一致  <br><br>• 用于校准 Model-based graders<br><br> | • 成本高<br><br>•  慢<br><br>• 往往需要一定规模的领域专家 |
